...
void HLTHiggsSubAnalysis::bookHistograms(DQMStore::IBooker &ibooker)
{
    ...
    for(size_t i = 0; i < sources.size(); i++)
    {
        ...
        if( sources[i] == "rec" ) _elements[nameVtxPlot] = ibooker.book1D(nameVtxPlot.c_str(), titlePu.c_str(), nBinsPu, minPu, maxPu);
        ...
        for (size_t j = 0 ; j < _hltPathsToCheck.size() ; j++){
            ...
            if( sources[i] == "rec" ) _elements[nameVtxPlot+"_"+shortpath] = ibooker.book1D(nameVtxPlot+"_"+shortpath, titlePassingPu.c_str(), nBinsPu, minPu, maxPu);
...
/*      if( it->first == EVTColContainer::MUON || it->first == EVTColContainer::TRACK )
            ...
            it != sourceMatchMap.end(); ++it)
    {
        ...
        if( it->first == RECO ) _elements[nameVtxPlot]->Fill(nbMCvtx);
                ...
                an != _analyzers.end(); ++an)
        {
            ...
            if (ispassTrigger) {
                ...
                if( it->first == RECO ) _elements[nameVtxPlot+"_"+fillShortPath.c_str()]->Fill(nbMCvtx);
